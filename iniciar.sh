...
on_error() {
    ...
    if [ -n "${REPO_DIR:-}" ] && [ -d "$REPO_DIR" ]; then
        echo ""
        echo "📁 Se detectó progreso ya descargado en:"
        echo "   $REPO_DIR"
        read -r -p "¿Deseas eliminarlo antes de salir? [s/N]: " resp
        case "$resp" in
            [sS]|[sS][iI])
                rm -rf -- "$REPO_DIR"
                echo "🗑️  Eliminado."
    ...
    if [ -n "${CARPETA_DESTINO:-}" ] && [ -d "$CARPETA_DESTINO" ]; then
        echo ""
        echo "📁 Se detectó una instancia ya creada en:"
        echo "   $CARPETA_DESTINO"
        read -r -p "¿Deseas eliminarla antes de salir? [s/N]: " resp2
        case "$resp2" in
            [sS]|[sS][iI])
                ...
                ;;
            *)
                echo "✅ Conservada en: $CARPETA_DESTINO"
                ;;
        esac
    fi
...
# servicio se llama literalmente "mysql", bash intenta expandir una
